...
cBotModule::~cBotModule() { dlclose(lib); }

bool cBotModule::ChannelMessage(const char *chan, const char *nick,
                                const char *user, const char *host,
                                const char *mess) {
  if (channel_message)
    return channel_message(chan, nick, user, host, mess);
  else
    return false;
};
bool cBotModule::ChannelNotice(const char *chan, const char *nick,
                               const char *user, const char *host,
                               const char *mess) {
  if (channel_notice)
    return channel_notice(chan, nick, user, host, mess);
  else
    return false;
};
bool cBotModule::ChannelAction(const char *chan, const char *nick,
                               const char *user, const char *host,
                               const char *mess) {
  if (channel_action)
    return channel_action(chan, nick, user, host, mess);
  else
    return false;
};
bool cBotModule::ChannelCtcp(const char *chan, const char *nick,
                             const char *user, const char *host,
                             const char *mess) {
  if (channel_ctcp)
    return channel_ctcp(chan, nick, user, host, mess);
  else
    return false;
}

bool cBotModule::PrivateMessage(const char *nick, const char *user,
                                const char *host, const char *mess) {
  if (private_message)
    return private_message(nick, user, host, mess);
  else
    return false;
}
bool cBotModule::PrivateNotice(const char *nick, const char *user,
                               const char *host, const char *mess) {
  if (private_notice)
    return private_notice(nick, user, host, mess);
  else
    return false;
}
bool cBotModule::PrivateAction(const char *nick, const char *user,
                               const char *host, const char *mess) {
  if (private_action)
    return private_action(nick, user, host, mess);
  else
    return false;
}
bool cBotModule::PrivateCtcp(const char *nick, const char *user,
                             const char *host, const char *mess) {
  if (private_ctcp)
    return private_ctcp(nick, user, host, mess);
  else
    return false;
}
