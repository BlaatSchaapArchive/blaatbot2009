...
char *bc_id(void) { return (char *)"blaat"; }
...
cBot::cBot(cProtocol *P) {
  ...
  if (ModuleToLoad.isEmpty())
    printf("No Modules In Config\n");
  ...
  if (targ && mynick)
    if (strcasecmp(targ, mynick)) {
      ...
      for (int i = 0; i < Mods.size(); i++) {
        if (Mods[i]->ChannelMessage(targ, nick, user, host, mess))
          break;
      }
    } else {
      printf("private message\n");
      for (int i = 0; i < Mods.size(); i++) {
        if (Mods[i]->PrivateMessage(nick, user, host, mess))
          break;
      }
    }
}
...
void cBot::part(char *targ, char *nick, char *user, char *host, char *mess) {
  ...
  if (targ && nick && user && host) {
    ...
    if (mess)
      printf(" reason : %s", mess);
    printf("\n");
  }
}
//-----------------------------------------------------------------------------
void cBot::kick(char *chan, char *kick, char *mess, char *nick, char *user,
                char *host) {
  printf("kick: ");
}
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
void cBot::channel_add(char *chann, char *nick, char *user, char *host,
...
//-----------------------------------------------------------------------------
void cBot::motd_missing() { JoinChannels(); }
//-----------------------------------------------------------------------------
void cBot::nickchangefail(char *oldn, char *badn, char *mess, char *numb) {}
//-----------------------------------------------------------------------------
void cBot::unknown_message(char *targ, char *nick, char *user, char *host,
                           char *mess) {
