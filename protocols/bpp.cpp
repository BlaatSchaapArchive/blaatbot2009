...
//-----------------------------------------------------------------------------
cConnection::cConnection(sConnectInfo *CI) { ConnectInfo = CI; }
//-----------------------------------------------------------------------------
cConnection::~cConnection() {
  if (ConnectInfo->host)
    free(ConnectInfo->host);
  if (ConnectInfo->ipv4)
    free(ConnectInfo->ipv4);
  if (ConnectInfo->ipv6)
    free(ConnectInfo->ipv6);
  //  if(ConnectInfo->port)      free(ConnectInfo->port);
  if (ConnectInfo->nick)
    free(ConnectInfo->nick);
  if (ConnectInfo->user)
    free(ConnectInfo->user);
  if (ConnectInfo->auth)
    free(ConnectInfo->auth);
  if (ConnectInfo->pass)
    free(ConnectInfo->pass);
  free(ConnectInfo);
}
//-----------------------------------------------------------------------------
int cConnection::Connect() {
  ...
  if (ConnectInfo->host) {
    ...
    if (hp) {
      ...
      memcpy((saServer6.sin6_addr.s6_addr),
             (((struct in6_addr *)(hp->h_addr_list[0]))->s6_addr),
             sizeof(in6_addr));
      char temp[INET6_ADDRSTRLEN];
      printf("%s\n", inet_ntop(AF_INET6, &(saServer6.sin6_addr), temp,
                               INET6_ADDRSTRLEN));
    }
