...
int prot_init(sConnectInfo *ConnectInfo) {
  printf("prot itit \n");
  if (!((ConnectInfo->host || ConnectInfo->ipv4 || ConnectInfo->ipv6) &&
        (ConnectInfo->port))) {
...
//------------------------------------------------------------------------------
void cIRCProtocol::back() { Connection->Send("AWAY\xd\xa"); }
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
void cIRCProtocol::raw(const char *raw) { Connection->Send(raw); }
//------------------------------------------------------------------------------
...
void cIRCProtocol::SendLogin() {
  ...
  char temp[666];
  sprintf(temp,
          "USER %s host server :BlaatSchaap Coding Projects 2009 IRC\xd\xa",
          user);
  raw(temp);
  if (pass) {
...
//------------------------------------------------------------------------------
void cIRCProtocol::SplitUserNickHost(char *data, char *&user, char *&nick,
                                     char *&host) {
  nick = strtok(data + 1, "!");
  user = strtok(NULL, "@");
  host = strtok(NULL, "");
...
void cIRCProtocol::Process(char *data) {
  ...
  if (strcasecmp(protdata[0], "PING") == 0) {
    ...
    if (strcasecmp(protdata[1], "PRIVMSG") == 0) {
          ...
          if (strlen(userraw) > 6) {
            if (strncasecmp(userraw + 1, "ACTION", 6) == 0) {
              if (userraw[strlen(userraw) - 1] == 0x01) {
                ...
                // a malformed ACTION
                bc_action(Client, target, nick, user, host, userraw);
                printf("Malformed ACTION received!! Parsing as real ACTION "
                       "anyways\n");
              }
            } else {
      ...
      //*bc_unknown_message )(const char * targ, const char * nick, const char *
      //user, const char * host, const char * mess);
